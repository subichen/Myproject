...
def patient_reg(request):
	...
	if cnt.count()==1:	
            ...
            print "l_id===================",l_id
            obj=patient_regis(
                lid=l_id,name=a,place=b,gender=c,ph_no=d,email=e
                )
            obj.save()
            print "registered"
...
def booking_hm(request):
    ...
    tvar=token_tb.objects.all().aggregate(Max('tokeno'))
    print tvar
    d=tvar['tokeno__max']
    d=d+1
    print d
    return render(request,"booking_frm_hm.html",{'tokenno':d})
def booking(request):
    ...
    if request.session.has_key('uid'):
        ...
        print d
        user=patient_regis.objects.get(lid=request.session['uid'])
        return render(request,"booking_pg.html",{"user_dt":user,'tokenno':d})
    return HttpResponse("<script>alert('Please Login');window.location.href='/index/';</script>")
...
def list_hosp(request):
    ...
    hos=hosptital_regis.objects.filter(location=loc)
    print "no of hospitals in ",loc,hos.count()
    nwspl=[]
    for i in hos:
        print "i.hid",i.hid
        dept=doctor_regis.objects.filter(hid=i.hid)
        for j in dept:
            print "spl",j.specialization
            ll=str(j.specialization)
            if ll not in nwspl:
                nwspl.append(ll)
    print "nwspl",nwspl
##    ob=serializers.serialize("json",doctor_regis.objects.filter(hid=i.hid))
    data["dt1"]=nwspl
    print data
    return JsonResponse(data,safe=False)
...
def list_dept(request):
    print "list_dept"
    data={}
    loc=request.GET.get("locc")
    spl=request.GET.get("spl")
    
    print "District___________________________________________",loc,spl
    hos=hosptital_regis.objects.filter(location=loc)
...
##    hosp_id=request.GET.get("hosp_id")
    ...
    for i in hos:
        print "i.hid",i.hid
        dept=doctor_regis.objects.filter(hid=i.hid,specialization=spl)
        for j in dept:
            print "spl",j.hid
            ll=str(j.hid)
            print "ll",ll
    ...
    for i in doc:
        print i
        h=hosptital_regis.objects.filter(hid=i)
        for j in h:
            lss.append(str(j.hosp_nm))
    print lss
    data["dt1"]=lss
    print data
    return JsonResponse(data,safe=False)
def list_dr(request):
    ...
    obj=hosptital_regis.objects.filter(hosp_nm=hosp_id)
    print obj
    for i in obj:
        g=i.hid
    ob=serializers.serialize("json",doctor_regis.objects.filter(specialization=dept,hid=g))
    print ob
    data["dt1"]=json.loads(ob)
    print data
    return JsonResponse(data,safe=False)
...
def do_booking(request):
    ...
    if request.method=="POST":
        if request.session.has_key('uid'):
            ...
            print "no of bookings",check_bk.count()
            if check_bk.count()<=2:
                hsp=hosptital_regis.objects.get(hosp_nm=bk_hos)
                dr=doctor_regis.objects.get(did=bk_dr)
                obj=booking_tb(
                    pid=request.session['uid'],did=bk_dr,location=bk_loc,specialization=bk_dist,time=bk_tym,date=bk_dt,hid=hsp.hid,hosp_name=hsp.hosp_nm,dr_name=dr.name,name=p_nm
                    )
                obj.save()
                print "booked"
                msgg="booked"
                ...
                d=d+1
                print type(d),d
                print type(bk_dr),bk_dr
                print type(hsp.hid),hsp.hid
                
                obj1=token_tb(
                    tokeno=d,pid=request.session['uid'],did=bk_dr,hid=hsp.hid
                    )
                obj1.save()
                llid=request.session['uid']
                return HttpResponse("<script>alert('Booked Successful');window.location.href='/dr_bk_msg/';</script>")
            else:
                return HttpResponse("<script>alert('Sorry booking closed for this slot,Please select another slot');window.location.href='/dr_bk_msg/';</script>")
...
def dr_bk_msg(request):
    if request.session.has_key('uid'):
        ...
        print d
        user_dt=patient_regis.objects.get(lid=request.session['uid'])
        return render(request,"booking_pg.html",{"user_dtt":user_dt,'tokenno':d})
    return HttpResponse("<script>alert('Please Login');window.location.href='/index/';</script>")
        ...
        if count==0:
            ...
            if count==1 and typ=="patient":
                ...
                if check_bk.count()<=2:
                    ...
                    obj1.save()
########                    ins=token_tb(
########                        tokeno=int(d),pid=request.session['uid'],did=int(bk_dr),hid=int(hsp.hid)
########                        )
########                    ins.save()
                    bk_loc=""
                    bk_hos=""
...
def updt_bkng_tym(request):
    ...
    if request.method=="POST":
        ...
        server.close()  
        return HttpResponse("<script>alert('Mail Send Successfully');window.location.href='/dr_Vw_bkngs/';</script>")
...
def addtest(request):
    print "addtest"
    user_dt=lab_regis.objects.get(lid=request.session['uid'])
    return render(request,"addtest.html",{"user_dtt":user_dt})
...
def dr_dt(request):
    ...
    ob=serializers.serialize("json",doctor_regis.objects.filter(did=dt))
